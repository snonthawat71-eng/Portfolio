...
(function () {
  ...
  var COLORS = ["255,255,255", "255,225,220", "255,150,140", "255,70,60"];
  ...
  function spawn(p) {
    // แผ่ออกจากหลังรูปเข้าหาโซนที่มองเห็น (ซ้าย/บน/ล่าง) — ลอยเอื่อย ๆ ละเอียด
    var ang = Math.PI * 0.45 + Math.random() * Math.PI * 1.1;
    var spd = rand(0.05, 0.20);                 // ช้า สงบ
    p.x = originX + rand(-8, 8);
    p.y = originY + rand(-8, 8);
    p.vx = Math.cos(ang) * spd;
    p.vy = Math.sin(ang) * spd;
    p.r = rand(0.4, 1.5);                        // จุดเล็ก ละเอียด
    p.life = 0;
    p.maxLife = rand(900, 2000);                // อยู่นาน ลอยช้า
    p.base = rand(0.3, 0.78);                   // ไม่จ้า
    p.tw = rand(0.006, 0.018);                  // กะพริบนุ่ม ช้า
  ...
  function makeParticles() {
    ...
    count = Math.max(70, Math.min(150, count));
    particles = [];
  ...
  function draw(p) {
    ...
    var fade = Math.min(prog / 0.15, (1 - prog) / 0.3, 1);     // จางเข้า/ออกตอนปลาย
    var twinkle = 0.5 + 0.5 * Math.sin(t * p.tw + p.seed);     // กะพริบนุ่ม
    var alpha = Math.max(0, fade) * p.base * (0.5 + 0.5 * twinkle);
    if (alpha <= 0.01) return;
    ctx.save();
    ctx.globalAlpha = alpha;
    ctx.shadowColor = "rgba(" + p.c + ",1)";         // แสงเรืองบาง ๆ
